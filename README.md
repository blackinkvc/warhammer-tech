...
- **站务日志**：站点内「日志」导航展示历次迭代（资源版本号 / 日期 / 协作 AI / 主要内容）；要点过多时默认收起，可展开查看完整条目；与仓库根 `CHANGELOG.md` 同步。
- **资料双视图**：资料页支持「卡片 / 列表」两种浏览模式切换。
...
index.html            # 站点入口
assets/
  data.js             # 全部数据（派系/概念/纪元/科技/参考/站务日志）
  app.js              # 视图渲染、关系图、搜索逻辑、站务日志视图
  style.css           # 卷宗视觉体系
CHANGELOG.md          # 站务日志（与站点「日志」视图同步）
tests/check.js        # DOM 桩回归校验
package.json          # npm run check（一键校验）
```
